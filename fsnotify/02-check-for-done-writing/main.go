...
func main() {
			...
			for {
				...
				case event, ok := <-watcher.Events:
					...
					if event.Op == fsnotify.Write {
						...
						mu.Unlock()
						if !ok {
							log.Printf("info: received Write, but no entry for %v in map\n", event.Name)
							continue
						}
						ch <- event
			...
			for {
				...
				case ch := <-eventCh:
					...
					go func(ch chan fsnotify.Event) {
						ticker := time.NewTicker(time.Millisecond * time.Duration(c.waitWrite))
						...
						for {
							...
							case event := <-ch:
								...
								ticker.Reset(time.Millisecond * time.Duration(c.waitWrite))
	...
	<-sigCh
	log.Printf("info: Ending all go routines\n")
